...
describe("SearchDirectory", () => {
  ...
  it("renders workers from Firestore collection data", () => {
    ...
    expect(mockUseCollection).toHaveBeenCalledWith("workerProfiles");
  });
  ...
  it("uses fallback worker display fields when a matching worker profile lacks them", () => {
    mockCollections({
      workerProfiles: [
        workerProfile({
          id: "worker-lina",
    ...
    const card = cardFor("Lina Morales");

    expect(card).toBeInTheDocument();
    expect(within(card as HTMLElement).getByText(/Electricidad/)).toBeInTheDocument();
    expect(mockUseCollection).toHaveBeenCalledTimes(1);
  ...
  it("uses safe generic display defaults when a worker profile lacks fields and has no fallback match", () => {
    ...
    expect(screen.getByRole("heading", { name: "Trabajador FixMySpace" })).toBeInTheDocument();
    expect(screen.getByText("Perfil profesional en construcción.")).toBeInTheDocument();
    expect(mockUseCollection).not.toHaveBeenCalledWith("users");
  });

  it("marks the source as fallback when workerProfiles is empty", () => {
    mockCollections({ workerProfiles: [] });

    render(<SearchDirectory />);

    expect(screen.getByText("Fallback visual")).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Lina Morales" })).toBeInTheDocument();
    expect(mockUseCollection).not.toHaveBeenCalledWith("users");
  });

  it("marks the source as fallback and shows an error when workerProfiles errors", () => {
    mockCollections({ workersError: "No pudimos leer los datos en Firestore." });

    render(<SearchDirectory />);

    expect(screen.getByText("Fallback visual")).toBeInTheDocument();
    expect(screen.getByText("No pudimos leer Firestore. Mostramos perfiles de ejemplo mientras revisas la conexión.")).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Lina Morales" })).toBeInTheDocument();
    expect(mockUseCollection).not.toHaveBeenCalledWith("users");
  });
});
